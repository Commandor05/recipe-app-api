...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_update_user_returns_error(self):
        """Test changing the recipe uesr results in an error."""
    ...
    def test_delete_other_users_recipe_error(self):
        ...
        new_user = create_user(email="user2@exaple.com", password="test123")
    ...
    def test_create_recipe_with_new_tags(self):
        ...
        for tag in payload["tags"]:
            tag_filter = recipe.tags.filter(name=tag["name"], user=self.user)
            self.assertTrue(tag_filter.exists())
    ...
    def test_create_tag_on_update(self):
        """Test creating tag when updationg a recipe."""
        recipe = create_recipe(user=self.user)
    ...
    def test_create_recipe_with_new_tests(self):
        ...
        for ingredient in payload["ingredients"]:
            ingredient_filter = recipe.ingredients.filter(
                name=ingredient["name"],
                user=self.user,
            )
            self.assertTrue(ingredient_filter.exists())
    ...
    def test_create_ingredient_on_update(self):
        ...
        payload = {"ingredients": [{"name": "Cheddar Cheese"}]}
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format="json")

        self.assertEqual(res.status_code, status.HTTP_200_OK)
        new_ingredient = Ingredient.objects.get(
            user=self.user, name="Cheddar Cheese"
        )
    ...
    def test_fileter_by_ingredients(self):
        """Test filtering recipes by ingredients."""
        recipe1 = create_recipe(user=self.user, title="Thai Curry")
        recipe2 = create_recipe(user=self.user, title="Aubergine with Tahini")
        ingredient1 = Ingredient.objects.create(user=self.user, name="Peanut")
        ingredient2 = Ingredient.objects.create(
            user=self.user, name="Eggplant"
        )
